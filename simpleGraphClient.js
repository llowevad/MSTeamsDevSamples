...
class SimpleGraphClient {
    ...
    async SendMessage(token, action) {
        const graphMessageBody = {
            "subject": action.messagePayload.subject,
            "body": action.messagePayload.body,
            "attachments": action.messagePayload.attachments
        };

        let graphMessageEndpoint = "https://graph.microsoft.com/v1.0/teams/9cfc4d00-a0da-403c-823f-3e384fb6e9d0/channels/19:fe73bcce17a44b1ea0950d45fdef6f96@thread.tacv2/messages";
        let graphMessageParams = {
            method: "POST",
            headers: {
                "Host": "graph.microsoft.com",
                "Content-Length": `${JSON.stringify(graphMessageBody).length}`,
                "Content-Type": "application/json",
                "Authorization": "bearer " + token
            },
            body: `${JSON.stringify(graphMessageBody)}`
        }

        let response = await fetch(graphMessageEndpoint, graphMessageParams).catch(this.unhandledFetchError);
        if (!response.ok) {
            console.error("ERROR: ", response);
        }
